...
const SITEMAP_PATH = 'data/sitemap.json';

async function _fetchSitemap() {
  // console.log('fetching sitemap');
  let sitemap = [];
  const resp = await fetch(`http://localhost:3001/tools/importer/${SITEMAP_PATH}`);
  if (resp.ok) {
...
async function _fsSitemap() {
  let sitemap = [];
  const dirname = new URL('.',import.meta.url).pathname;
  try {
    const fs = await import('fs');
    const f = fs.readFileSync(`${dirname}/${SITEMAP_PATH}`);
    sitemap = JSON.parse(f);
  } catch (err) {
    console.log(`Unable to load sitemap from file ${dirname}/${SITEMAP_PATH}. ${err.message}`, err);
  }
  return sitemap;
}
...
async function _sitemap() {
  // console.log('fetching sitemap');
  let sitemap = (typeof process === 'object') ? await _fsSitemap() : await _fetchSitemap();
  return sitemap;
}
...
async function updateLink(el, url, rewrites, err) {
  ...
  if (shouldRewriteLink(href)) {
    ...
    if (href) {
      ...
      let proxyUrl = (port === 80 || port === '') ? `${hostname}${oldPath}` : `${hostname}:${port}${oldPath}`;
...
function transformProductDOM(document, templateConfig) {
  ...
  productContent.forEach((el) => {
    ...
    if (media)  {
      if (el.querySelector('.productTitle')) {
        el.querySelector('.productTitle').insertAdjacentElement('afterend', media);
      } else {
        el.insertAdjacentElement('beforeend', media);
      }
    }
...
function transformCourseListingDOM(document, templateConfig) {
  ...
  const metadata = getOrCreateMetadataBlock(document, main);
  appendMetadata(metadata, 'og:type', 'article');
  appendMetadata(metadata, 'template', templateConfig.template);
  appendMetadata(metadata, 'category', templateConfig.category);
  appendPageMetadata(document, metadata);
  return {
